        ...
        <div className={`rounded-2xl border ${isDarkMode ? 'glass-card border-gray-700/50' : 'bg-white border-gray-200 shadow-sm'}`}>
          ...
          {!loading && trashedMaterials.length === 0 && (
            <div className="p-16 text-center">
              <svg
                className={`mx-auto h-16 w-16 mb-4 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <h3 className={`text-xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Trash is empty</h3>
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Deleted materials will appear here and automatically delete after 30 days
              </p>
            </div>
          )}
