      ...
      <div className={`toolkit-modal-content ${isDarkMode ? 'dark-mode' : ''}`} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center text-white flex-shrink-0`}>
              {activeTab === 'lock' ? <FaLock className="w-5 h-5" /> : <FaUnlock className="w-5 h-5" />}
            </div>
            <div>
              <h2>Secure PDF</h2>
              <p className={`text-sm mt-0.5 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                {activeTab === 'lock' ? 'Encrypt your PDF with a password' : 'Remove password protection'}
              </p>
            </div>
          </div>
